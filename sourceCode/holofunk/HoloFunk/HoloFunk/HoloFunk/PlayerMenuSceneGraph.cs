...
namespace Holofunk
{
    ...
    class PlayerMenuSceneGraph<TBaseModel> : SceneGraph
    {
        ...
        internal PlayerMenuSceneGraph(PlayerSceneGraph parent, PlayerMenuModel menuModel, Vector2 center)
            : base()
        {
            m_parent = parent;
            m_model = menuModel;
            ...
            for (int i = 0; i < menuModel.MenuItemCount; i++) {
                Transform menuItemLocation = new Transform(GetLocation(i));
        ...
        internal Vector2 GetLocation(int index)
        {
            ...
            int textureRadius = (int)(m_parent.TextureRadius * MagicNumbers.LoopieScale);
            ...
            Moment now)
        {
            for (int i = 0; i < m_menuItemNodes.Count; i++) {
                m_menuItemNodes[i].SetState(TextSpriteNodeState.Default);
            }
            if (m_model.SelectedMenuItem.HasValue) {
                m_menuItemNodes[m_model.SelectedMenuItem.Value].SetState(TextSpriteNodeState.Highlighted);
            }
        }
    }
}
